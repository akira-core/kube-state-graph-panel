...
| `p90` | `metrics.p90ServerMs` | `< 1000` 時 `<value> ms`;`>= 1000` 時換算為 `<value/1000> s` |
...
- `metrics` 存在但 `errorRate` 不存在時 MUST NOT 顯示 `errorRate` row(**尤其 MUST NOT 顯示 `0%`**:省略代表「未能量測」,與量測到 0 是不同狀態)。`p90ServerMs` 不存在時同理不顯示 `p90` row。
...
- **THEN** tooltip 依序顯示 `edgeType: pod-calls-service`、`rate: 5 req/s`、`errorRate: 20%`、`p90: 45 ms`
...
- **THEN** tooltip 僅追加 `rate: 3 req/s` 一列;MUST NOT 出現 `errorRate` 或 `p90` row
...
- **THEN** `p90` row 顯示 `2.5 s`(而非 `2500 ms`)
