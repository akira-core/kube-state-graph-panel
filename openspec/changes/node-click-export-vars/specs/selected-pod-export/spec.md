...
**Reason**: status gating(僅 warning/critical 匯出)移除——本 capability 的用途從 alert-driven 單 pod 檢視擴充為通用 log 過濾:任何 pod / controller 點擊皆匯出。
**Migration**: 由新 requirements「左鍵點擊 pod 匯出名稱與 cluster」「左鍵點擊 controller 匯出全部子 pod 名稱與 cluster」取代;消費端 dashboard 無需改變數名(`selectedPodVariable` 沿用),但會在 normal pod 點擊時收到值(原為清除)。

### Requirement: 寫入重用單一寫入路徑(單值 + 哨兵 + 等值跳過)

**Reason**: 寫入語意由「單值單變數」擴充為「單/多值 + 雙變數」,requirement 名稱與內容一併更新。
**Migration**: 由新 requirement「寫入重用單一寫入路徑(單/多值 + 哨兵 + 等值跳過)」取代,哨兵、等值跳過、`replace: true` 防護全數保留。
