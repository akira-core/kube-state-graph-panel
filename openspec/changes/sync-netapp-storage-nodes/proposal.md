...
- **Storage `metrics` on that edge** — `read_ops` / `write_ops` / `read_latency_us` / `write_latency_us` / `read_bytes_per_sec` / `write_bytes_per_sec`, hoisted through the same normalize path as RED and surfaced in the tooltip. The two throughput fields are bytes per second and format through the shared decimal byte-unit ladder (`5.24 MB/s`), the same one the node `usage` row already uses. `data.metrics` becomes a **union**: an edge carries either the RED family (`rate` REQUIRED within it) or the I/O family, never both, so `rate` can no longer be assumed present.
...
**Demo.** `dev/victoriametrics/seed.sh` gains the Harvest volume / aggregate / node series plus kubelet volume stats, and `docker-compose.yaml`'s default `KSG_BACKEND_TAG` must point at an image carrying the backend change.
