...
- **Storage `metrics` on that edge** — `read_ops` / `write_ops` / `read_latency_us` / `write_latency_us`, hoisted through the same normalize path as RED and surfaced in the tooltip. `data.metrics` becomes a **union**: an edge carries either the RED family (`rate` REQUIRED within it) or the I/O family, never both, so `rate` can no longer be assumed present.
