...
### Requirement: compound 容器的 icon 擺放

當 `node` 或 `controller` 節點身為 compound parent(其下有子節點)時,系統 MUST 將其 icon 縮小並置於容器左上角(貼近 label),不置中,以免被子節點覆蓋;leaf 或 collapsed 狀態的同類節點 icon 置中。`cluster` 容器(`isCluster`)MUST NOT 渲染 resource icon。因 cytoscape 的 `background-image` 節點無法可靠 revert 為純 `background-color`,「無 icon」MUST 以透明/空白影像表示,而非移除 `background-image` 屬性。

#### Scenario: 展開的 controller 容器 icon 置左上

- **WHEN** 在 `controller` 模式下,某 Deployment 容器內含可見 pod 子節點
- **THEN** 該 Deployment 的 icon 縮小顯示於容器左上角(不置中),中央區域留給子節點

#### Scenario: cluster 容器不放 resource icon

- **WHEN** 渲染 `type: 'cluster'` 的 compound 容器
