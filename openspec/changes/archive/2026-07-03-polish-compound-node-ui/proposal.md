...
- Compound group labels gain a title-case kind prefix + a space after the colon: a cluster named `prod` renders as `Cluster: prod`, a namespace `checkout` as `Namespace: checkout`, an application `mongo` as `Release Unit: mongo` (the `application` kind's display prefix is renamed "Release Unit"; its internal type/kind string, `isApplication` flag, and `applicationColor` are unchanged). Needed because colour no longer disambiguates individual clusters/namespaces/applications from each other — the label now carries that signal instead.
- The two OTHER compound-box headers — the **physical-network fabric box** (`kind: network`, wrapping the switches) and the **k8s `node` box** (the pod-wrapping compound in node-layout mode) — are aligned to the same capitalisation + enlarged label. The physical-network name is title-cased (`physical network`→`Physical Network`); the k8s node gains a `Node: ` prefix (`Node: worker-0`) mirroring the group prefixes. Both are applied as **render-only** stylesheet function `label` mappers (NOT baked into `data.label`), because a k8s node's `data.label` is its identity value: the dashboard `name=` query param and the detail-panel title read it directly, so a baked prefix would corrupt the query and duplicate the kind badge. `font-size` is bumped to match the group headers (network 17, node 18). The k8s-node header treatment applies **only when the node is an actual compound** (`node[kind='node']:parent`, plus a folded-compound sibling): in controller-layout the node is a plain leaf and keeps the normal node title (bare label, base font). Switch leaf nodes are left unchanged.
