...
- **THEN** `podParentMode` 為 `'controller'`,layout 分段控制預設高亮 `Controller`;pod 巢狀於其 owning controller(`cluster > controller > pod`)、`controller-owns-pod` 不繪製(以巢狀表示)、合成 `pod-runs-on-node` 為 drawn edge;且圖中所有 controller 容器於初次載入即預設全摺疊(pod 聚合)
