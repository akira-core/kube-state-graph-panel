...
- **Legend panel collapse:** add a `<` 收合鈕 at the top of the legend `<aside>` (right-aligned) that hides the entire legend to the side so the canvas reclaims the freed width; while hidden, a single floating `>` 還原鈕 (top-left, over the canvas) restores it. State is panel-local and ephemeral (`useState`, not persisted to panel options); the whole affordance is still gated on `options.showLegend` (option off ⇒ no legend, no toggle buttons).
...
- `panel-rendering`: the decorative `cluster` / `namespace` / `application` groups change from non-selectable to selectable (so the built-in collapse cue can render on them); they still MUST NOT open the node-detail panel. The legend panel additionally gains a collapse / restore toggle.
...
- `src/panels/KsgPanel/KsgPanel.tsx` — a panel-local `legendCollapsed` `useState`; a `<` `IconButton` inside the legend `<aside>` and a floating `>` `IconButton` over `canvasArea`; one new style for the floating restore button. `KsgPanel.test.tsx` covers the collapse → hide → restore cycle.
