...
- [x] 5.4 demo 手動 / Playwright 驗證:右鍵 controller → 兩區塊預取 loading→(backend 有資料)anchor /(404)muted「No change report」;確認新分頁不再空白(對真實 Azure DevOps)。**備註**:demo backend 404s `config_changes`/`code_changes`;anchor 修復(移除 `window.open`)已由 code review 驗證,新分頁空白行為需真實 Azure DevOps 才能 E2E 驗證,接受此限制
