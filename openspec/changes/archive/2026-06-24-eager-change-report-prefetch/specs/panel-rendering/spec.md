...
- **unavailable(失敗 / 查無 / 無 URL)**:
  - **Application**:`config_changes` 失敗(非 200 / 回應格式錯誤 / 無有效 url)時,Change Report 欄 MUST 以次要(muted)文字顯示「Not found」提示(MUST NOT 渲染 anchor / 按鈕;過長截斷、完整失敗訊息入 `title` 以保留錯誤可見性)。
...
- **THEN** 該列 Change Report 欄顯示「Not found」提示(無 anchor),name 與 image 仍照常顯示

#### Scenario: 查詢失敗顯示「Not found」且不波及其餘

- **WHEN** `config_changes`(或 `code_changes`)查詢失敗(非 200 / 網路錯誤 / 回應格式錯誤)
- **THEN** 對應目標 Change Report 欄以次要色顯示「Not found」提示(無 anchor;過長截斷、完整失敗訊息入 `title` 以保留錯誤可見性)
...
- **THEN** 右鍵開啟的 detail view 中兩區塊照資料渲染,Change Report 欄顯示「Not found」提示(`enabled` 為 false),且 MUST NOT 發出任何查詢
