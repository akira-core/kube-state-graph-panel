...
- [x] 7.3 ~~Bump the default `KSG_BACKEND_TAG`.~~ **Superseded by `render-from-fixture-only`**, which removed the backend service, the seeder, and VictoriaMetrics from `docker-compose.yaml` entirely. There is no image tag left to bump: the demo's storage half is rendered from the typed fixture, so it can never fall behind an unpublished backend build.
...
- [x] 11.1 `src/shared/types/cytoscape.d.ts`: add `maxIops?: number` / `maxBytesPerSec?: number` to `EdgeIoMetrics`, recording on the fields that `maxBytesPerSec` is bytes per second (already converted upstream from Harvest's MB/s) and that absence means *no declared ceiling* — never `0`, never an unlimited sentinel.
- [x] 11.2 `normalize.ts` `parseEdgeMetrics`: hoist `max_iops` / `max_bytes_per_sec` with the same per-field guard as the existing six; either field alone MUST keep the I/O family alive, and normalize MUST NOT enforce "a ceiling never appears alone" (that invariant is the backend's). Extend `normalize.test.ts`: both ceilings present, measurements-with-no-ceiling, one ceiling invalid (drops that field only), values passed through unconverted.
- [x] 11.3 `src/features/hover-tooltip/formatEdgeMetrics.ts`: route `maxIops` through the existing ops formatter and `maxBytesPerSec` through `formatThroughputBytesPerSec` (added in 9.3) — no new formatter, that shared ladder is what makes a ceiling and a measurement comparable at a glance.
- [x] 11.4 `HoverTooltip`: append `max iops` / `max throughput` rows **after** the two throughput rows, each guarded by `isFiniteNumber`, neutral-coloured. Exceeding a ceiling MUST NOT colour or warn. Extend `HoverTooltip.test.tsx` with the eight-row storage edge, the measured-but-uncapped case, and an over-ceiling case asserting neutral colour.
- [x] 11.5 ~~Reshape `dev/victoriametrics/topology.prom` onto the three-hop Harvest model.~~ **Superseded by `render-from-fixture-only`**, which deleted `dev/victoriametrics/` with the rest of the backend stack. The capped/uncapped contrast this task existed to produce now lives in `src/shared/fixtures/showcaseGraph.ts` as two `pvc-to-netapp-aggr` edges, pinned by `showcaseGraph.test.ts`. Original text:
...
- [x] 11.6 ~~Seed the QoS policy families for one volume only.~~ **Superseded** — same reason as 11.5; the fixture carries `max_iops` / `max_bytes_per_sec` on the aggr1 edge and deliberately omits them on the aggr2 edge. Original text:
...
- [x] 11.7 ~~Re-run the 7.4 visual check against a backend image carrying `160c347`.~~ **Superseded** — there is no backend image to run against. The equivalent check is `docker compose up -d` against the generated showcase dashboard. Original text:
...
- [x] 11.8 ~~Correct the seeded Harvest family names in `CLAUDE.md`.~~ **Superseded** — `render-from-fixture-only` rewrote that whole section: there are no seeded families left to name. Original text:
  > 11.8 `CLAUDE.md` demo-fixture description: correct the seeded Harvest family names to the three-hop set.
- [x] 11.9 Full gate: `npm run lint && npm run typecheck && npm run test:ci && npm run build` clean; `openspec validate sync-netapp-storage-nodes --strict`.
