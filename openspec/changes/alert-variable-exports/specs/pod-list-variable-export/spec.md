# pod-list-variable-export — Delta (alert-variable-exports)

## REMOVED Requirements

### Requirement: Panel option 指定目標變數(預設停用)

**Reason**: option 名稱 `podListVariable` 寫死在需求文中;option 更名為 `alertPodListVariable` 且新增第二個 option `alertNameListVariable`,需求重寫為雙 option 版本。
**Migration**: 由新 requirement「Panel options 指定兩個目標變數(預設停用)」取代。使用者 dashboard 的 panel options 需把變數名改填到 `alertPodListVariable`(舊 key `podListVariable` 不再讀取,**BREAKING**)。

### Requirement: Pod 名稱擷取(資料層、全量)

**Reason**: 「全量」語意廢止 — 清單改為只含帶有 alert 的 pod,並新增 alert 名稱擷取。
**Migration**: 由新 requirements「Alert pod 名稱擷取(資料層)」與「Alert 名稱擷取(資料層、跨節點種類)」取代;資料層輸入不變(normalize 直接輸出、視圖狀態無關)。
